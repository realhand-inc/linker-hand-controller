...
class LinkerHandL20Can:
    def __init__(self, can_channel='can0', baudrate=1000000, can_id=0x28,yaml=""):
        ...
        atexit.register(self.close_can_interface)
    
    def __del__(self):
        # Best-effort cleanup to avoid socket warnings on interpreter exit
        try:
    ...
    def _create_bus(self):
        ...
        if sys.platform == "linux":
            # Ensure the interface is up before creating the bus
            self.open_can.open_can(self.can_channel)
            # Do not apply CAN filters here: some firmware replies on a
            # different arbitration ID than the request. Accept all frames and
            # filter in software instead to avoid missing responses.
    ...
    def _ensure_bus_ready(self):
        """Make sure the CAN bus exists and its socket is open."""
        try:
            if self.bus and getattr(self.bus, "socket", None):
                if self.bus.socket.fileno() >= 0:
                    return True
            # Previous bus is invalid, try to recreate
            if self.bus:
                try:
                    self.bus.shutdown()
                except Exception:
            ...
            print("Please insert CAN device or bring up CAN interface", flush=True)
            self.bus = None
            return False
    ...
    def receive_response(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Bus socket was closed; try to recreate
                self._ensure_bus_ready()
                time.sleep(0.2)
            except can.CanError as e:
    ...
    def send_command(self, frame_property, data_list,sleep=0.002):
        if not self._ensure_bus_ready():
            print("CAN bus not ready, cannot send command", flush=True)
            return
        frame_property_value = int(frame_property.value) if hasattr(frame_property, 'value') else frame_property
        data = [frame_property_value] + [int(val) for val in data_list]
