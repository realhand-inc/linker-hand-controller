...
class LinkerHandApi:
    def __init__(self, hand_type="left", hand_joint="L10", modbus = "None",can="can0"):  # Ubuntu:can0   win:PCAN_USBBUS1
        ...
        # Ensure CAN is closed even if the caller forgets
        atexit.register(self.close_can)
    ...
    def close_can(self):
        """Properly close all CAN/RS485 connections"""
        # Close the hand's CAN/RS485 interface first
        if self.hand:
            if hasattr(self.hand, 'close_can_interface'):
                # For CAN-based hands
                self.hand.close_can_interface()
            elif hasattr(self.hand, 'close'):
                # For RS485-based hands
                self.hand.close()

        # Then close the system CAN interface if it exists
        if hasattr(self, 'open_can') and self.open_can:
            try:
                self.open_can.close_can(self.can)
            except Exception:
                # Fallback for older helper naming
